...
class THORDiscreteEnvironment(object):
  ...
  def step(self, action):
    ...
    self.s_t1 = np.append(self.s_t[:,:,1:], self.observation, axis=2) #concateting a new state
  ...
  def _tiled_state(self, state_id):
    ...
    img_o = cv2.cvtColor(I, cv2.COLOR_RGB2GRAY )
    img_r = cv2.resize(img_o, (110, 110))/255
  ...
  def observation(self):
    ...
    img_g = cv2.cvtColor(new_image, cv2.COLOR_RGB2GRAY )
    img_s = cv2.resize(img_g, (110, 110))/255 #scaled new image
