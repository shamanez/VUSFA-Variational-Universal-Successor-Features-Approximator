...
class ActorCriticFFNetwork(ActorCriticNetwork):
  ...
  def __init__(self,
    ...
    with tf.device(self._device):
      ...
      # state (input)
      self.s = tf.placeholder("float",[None,110,110,4])

      # target (input)
      self.t = tf.placeholder("float",[None,110,110,4])
      ...
      with tf.variable_scope(network_scope):
        ...
        # flatten input
        self.s_flat = self.s
        self.t_flat =self.t        #Shared Siames Layer_cnn_1_var
        self.W_k1[key] = self._conv_weight_variable([8,8,4,32])  
        self.b_k1[key] = self._conv_bias_variable([32],8,8,4)



        #sharead Siames Layer_cnn_2_var
        ...
        self.w_fc_cnn[key] = self._fc_weight_variable([3200, 512])
        self.b_fc_cnn[key] = self._fc_bias_variable([512], 3200)
        ...
        s_flat = tf.nn.relu(tf.matmul(CNN_s_flat, self.w_fc_cnn[key]) + self.b_fc_cnn[key])
        #############################################################################################################


        #convoluting the target
        ############################################################################################################
        CNN_t_layer_1=tf.nn.leaky_relu(self._conv2d(self.t_flat,self.W_k1[key],4)+self.b_k1[key])
        CNN_t_layer_2=tf.nn.leaky_relu(self._conv2d(CNN_t_layer_1,self.W_k2[key],2)+self.b_k2[key])
        CNN_t_layer_3=tf.nn.leaky_relu(self._conv2d(CNN_t_layer_2,self.W_k3[key],2)+self.b_k3[key])

        CNN_t_flat=tf.contrib.layers.flatten(CNN_t_layer_3)
        t_flat = tf.nn.relu(tf.matmul(CNN_t_flat , self.w_fc_cnn[key]) + self.b_fc_cnn[key])
        ############################################################################################################
  ...
  def run_policy_and_value_t(self, sess, state, target, scopes):
    k = self._get_key(scopes[:2])
