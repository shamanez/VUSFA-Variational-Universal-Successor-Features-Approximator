...
class A3CTrainingThread(object):
  ...
  def process(self, sess, global_t, summary_writer, summary_op, summary_placeholders):
    ...
    if terminal:
        value_td,sr_td=sess.run( [self.local_network.summary_Value_loss,self.local_network.summary_SR_loss],
             feed_dict = {
        ...
        with open('value_td.csv','a+') as fp:
          wr = csv.writer(fp)
          wr.writerow(list([value_td]))
        with open('sr_td.csv','a+') as fp:
          wr = csv.writer(fp)
          wr.writerow(list([sr_td]))
