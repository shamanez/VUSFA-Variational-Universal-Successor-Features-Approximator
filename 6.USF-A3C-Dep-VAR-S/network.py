...
class ActorCriticNetwork(object):
  ...
  def prepare_loss(self, entropy_beta, scopes):
    ...
    with tf.device(self._device):
      ...
      mus_t=self.mus_s_loss_t[scope_key_b]
      sigmas_t=self.sigmas_s_loss_t[scope_key_b]

      mus_s=self.mus_s_loss[scope_key_b]
      sigmas_s=self.sigmas_s_loss[scope_key_b]

      mus=tf.concat(values=[mus_t,mus_s],axis=0)
      sigmas=tf.concat(values=[sigmas_t,sigmas_s],axis=0)
