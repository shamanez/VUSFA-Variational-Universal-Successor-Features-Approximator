...
from constants import CHECKPOINT_DIR_beta
...
if __name__ == '__main__':
  ...
  saver_pre = tf.train.Saver(max_to_keep=10, var_list=global_network.get_vars_pre())
  saver_b = tf.train.Saver(max_to_keep=10, var_list=global_network.get_beta_var())
 
  #saver = tf.train.Saver(max_to_keep=10)

  checkpoint = tf.train.get_checkpoint_state(CHECKPOINT_DIR)
  if checkpoint and checkpoint.model_checkpoint_path:
    saver_pre.restore(sess, checkpoint.model_checkpoint_path)
  ...
  checkpoint_b = tf.train.get_checkpoint_state(CHECKPOINT_DIR_beta)
  if checkpoint_b and checkpoint_b.model_checkpoint_path:
    saver_b.restore(sess, checkpoint_b.model_checkpoint_path)
    print("checkpoint loaded: {}".format(checkpoint_b.model_checkpoint_path))
    tokens_b = checkpoint_b.model_checkpoint_path.split("-")
    # set global step
    global_t_b = int(tokens_b[1])
    print(">>> global step set beta: {}".format(global_t_b))
  else:
    print("Could not find old checkpoint beta")
  ...
  def train_function(parallel_index):
    ...
    while global_t < MAX_TIME_STEP and not stop_requested:
      ...
      if parallel_index == 0 and global_t - last_global_t > 1000000:
        ...
        saver_b.save(sess, CHECKPOINT_DIR_beta + '/' + 'checkpoint', global_step = global_t)
