...
class A3CTrainingThread(object):
  def __init__(self,
    ...
    self.sync_beta_from_glob = self.local_network.sync_beta_from_glob(global_network)

    self.sync_beta_to_glob_from_local = self.local_network.sync_beta_to_glob(global_network)
  ...
  def process(self, sess, global_t, summary_writer, summary_op, summary_placeholders):
    ...
    sess.run( self.sync )
    sess.run( self.sync_beta_from_glob)
    ...
    for i in range(LOCAL_T_MAX):
      ...
      usf_s_g = self.local_network.run_usf(sess, self.env.s_t, self.env.target, self.scopes)


      pi_, value_ = self.local_network.run_policy_and_value(sess, self.env.s_t, self.env.target,usf_s_g, self.scopes)
    ...
    for(ai, ri, si, Vi, ti,state,usf_n,usf_p) in zip(actions, rewards, states, values, targets,state_representation_next,usf_next,usf):
      R = ri + GAMMA * R
    ...
    bottleneck_loss=sess.run( self.local_network.bottle_neck,
              feed_dict = {
                self.local_network.s: batch_si,self.local_network.t: batch_t} )  
    ...
    self.local_network.run_beta_ass_op(sess,bottleneck_loss , self.scopes)

    #Z=self.local_network.get_beta_var() 

 
    if terminal:
      print("Bottleneck Loss:",bottleneck_loss,"Beta:",sess.run(self.local_network.get_beta_var()))
  




    sess.run(self.sync_beta_to_glob_from_local)
    ...
    sess.run( self.accum_gradients,
              feed_dict = {
                ...
                self.local_network.usf_policy:batch_usf,
