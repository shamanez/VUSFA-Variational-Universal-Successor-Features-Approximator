...
class ActorCriticNetwork(object):
  ...
  def prepare_loss(self, entropy_beta, scopes):
    ...
    with tf.device(self._device):
      ...
      SR_loss = 0.005*tf.losses.mean_squared_error(self.return_usf ,self.usf[scope_key])
  ...
  def __init__(self,
    ...
    with tf.device(self._device):
      ...
      with tf.variable_scope(network_scope):
        ...
        h_t_flat = tf.matmul(h_t_flat_1, self.W_fc1_s1[key]) + self.b_fc1_s1[key]
        ...
        for scene_scope in scene_scopes:
          ...
          with tf.variable_scope(scene_scope):
            ...
            # policy (output)
            pi_ = tf.matmul(pi_h, self.W_policy[key]) + self.b_policy[key]
